#' Plot Skew-T and wind plots on a single layout 
#' 
#' Jakis dluzszy opis tego co tutaj sie bedzie dzialo... Ala ma kota. ALa ma kojota
#' 
...
#' @param title title to be added in the header
...
#' layout(pressure, altitude, temp, dpt, wd, ws)
#' 
#'

layout <- function(pressure, altitude, temp, dpt, wd, ws,
                 convert = FALSE, ptop = 100, interpolate = TRUE,
                 title = "Sample title", parcel = "MU", ...){
  ...
  RH <- dewpointdepression2rh(output$pressure*100, output$temp+273.15, output$temp-output$dpt, consts = export_constants())
  SH <- rh2shum(output$pressure*100, output$temp+273.15, RH, consts = export_constants())
  E <- q2e(output$pressure*100, SH, consts = export_constants())
  W <- e2w(E, output$pressure*100, consts = export_constants())
  TLCL <- boltonTLCL(output$temp+273.15, RH, consts = export_constants())
  THETAE <- equivalentPotentialTemperature(output$pressure*100, output$temp+273.15, W, TLCL, consts = export_constants())
  ...
  ####
  parametry <- sounding(pressure, altitude, temp, dpt, wd, ws)
  ...
  mtext(expression(paste(bold("thundeR")," - rawindsonde processing tool for R v1.0")), 
        line = -1.5, cex=0.8)
  #mtext("Ⓒ Piotr Szuster, Bartosz Czernecki, Mateusz Taszarek", line = -1.6, adj = 0, cex=0.62)
  #box()
  ###
  ...
  hodograph(u, v, output$altitude-output$altitude[1],max_speed = 25, frame=F)
  box()
