...
sounding_compute = function(pressure, altitude, temp, dpt, wd, ws, accuracy = 2, interpolate_step = 5, meanlayer_bottom_top = c(0,500), storm_motion = c(999,999)){
  ...
  ws2 = storm_motion[1]
  wd2 = storm_motion[2]
  storm_motion[1] = round(ws2 * 0.514444 * sin(wd2 * pi/180), 2)
  storm_motion[2] = round(-ws2 * 0.514444 * cos(wd2 * pi/180), 2)
  }  
  
  tmp = sounding_default(pressure, altitude, temp, dpt, wd, ws, export_profile, accuracy, interpolate_step, meanlayer_bottom_top, storm_motion = c(storm_motion[1],storm_motion[2],999))
