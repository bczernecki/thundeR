...
```{r skewt_customized, eval=TRUE, fig.width=7, fig.height=8}
library(thunder)
...
```{r change_par_wind_speed_profile, message=FALSE, warning=FALSE, include=FALSE}
oldpar = par()
```

```{r wind_speed_vertical_profile, eval=TRUE, fig.fullwidth=TRUE, fig.height=6, fig.width=8, warning=FALSE}
northplatte = get_sounding(wmo_id = 72562, yy = 1999, mm = 7, dd = 3, hh = 00)
...
```{r change_par_wind_speed_profile2, message=FALSE, warning=FALSE, include=FALSE}
par(oldpar)
```
...
```{r hodo_customized, eval = TRUE, fig.width=6, fig.height=6, fig.fullwidth = TRUE}
data("northplatte")
...
```{r options_restore, message=FALSE, warning=FALSE, include=FALSE}
options(old_options)
```
